def determine_user_birthday
  puts "When is your birthday? Use MMDDYYYY format."
  number = gets
end

def determine_birthpath_number
  number = determine_user_birthday
  number = (number[0].to_i + number[1].to_i + number[2].to_i + number[3].to_i + number[4].to_i + number[5].to_i + number[6].to_i + number[7].to_i_).to_s
...
when 9
  ...
  puts "Mmm, your birthday doesn't add up, no numerology number for you!"
end